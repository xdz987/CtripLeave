...
class Vocation extends Model {
	...
	public function get_ranking($user_vocation) {
		...
		//增加用户输入数据
		db('user')->where('industry', $user_vocation['industry'])->setInc('mem_sum', 1);
		db('user')->where('industry', $user_vocation['industry'])->setInc('account_year_day', $user_vocation['vo_year_day']);
		db('user')->where('industry', $user_vocation['industry'])->setInc('account_sum_day', $user_vocation['vo_sum_day']);

		if ($old_user_vocation) {
			//删除用户输入数据
			db('user')->where('industry', $user_vocation['industry'])->setDec('mem_sum', 1);
			db('user')->where('industry', $user_vocation['industry'])->setDec('account_year_day', $old_user_vocation['vo_year_day']);
			db('user')->where('industry', $user_vocation['industry'])->setDec('account_sum_day', $old_user_vocation['vo_sum_day']);
		}

		//查询所有行业数据
		$industry_rankRes = Db::table($tableName)->limit(10)->select();
		...
		$user_vocation['nickname'] = db('user')->field('nickname')->where('id', $user_vocation['uid'])->find();
	...
	public function user_vocation_operate($user_vocation) {
		...
		$old_user_vocation = db('vocation')->where('uid', $user_vocation['uid'])->find();
		if (!$old_user_vocation) {
			db('vocation')->insert($user_vocation);
		} else {
			db('vocation')->where('uid', $user_vocation['uid'])->update($user_vocation);
